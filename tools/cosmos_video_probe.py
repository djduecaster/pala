#!/usr/bin/env python3
"""Live camera -> Cosmos video probe for temporal/spatial reasoning characterization."""
...
def _build_payload(
    *,
    ...
    user_text = (
        f"{question}\n"
        "Frames are in chronological order (oldest first, newest last). "
        f"context={json.dumps(user_context, separators=(',', ':'), ensure_ascii=True)}"
    )

    content: list[dict[str, Any]] = [{"type": "text", "text": user_text}]
    for i, (image_data_url, age_ms) in enumerate(zip(image_data_urls, frame_offsets_ms), start=1):
        content.append(
            {
                "type": "text",
                "text": f"frame_index={i}/{len(image_data_urls)} frame_age_ms={age_ms:.1f}",
            }
        )
    ...
    if task == "describe":
        system_prompt = (
            "You are a vision assistant analyzing a short frame sequence from a fixed camera. "
            "Use temporal changes across frames to infer what is happening in the scene. "
            "Respond in 2-4 short sentences and mention motion/trends if visible. "
    ...
    system_prompt = (
        ...
        "Use temporal frame changes (not a single frame) when choosing action. "
...
def _sample_frames(
    ...
    *,
    now_mono_ns: int,
    max_frames: int,
) -> tuple[list[np.ndarray], list[float], float, list[list[int]]]:
    entries = list(history)
    if not entries:
        return [], [], 0.0, []

    if len(entries) > max_frames:
        step = len(entries) / float(max_frames)
    ...
    offsets_ms = [max(0.0, (now_mono_ns - entry[0]) / 1_000_000.0) for entry in entries]
    window_span_ms = 0.0
    if len(entries) >= 2:
        window_span_ms = max(0.0, (entries[-1][0] - entries[0][0]) / 1_000_000.0)
    return frames, offsets_ms, window_span_ms, shapes


def main() -> int:
    parser = argparse.ArgumentParser(description="Live video-sequence probe against Cosmos endpoint")
    ...
    parser.add_argument(
        ...
        default="describe",
        help="Probe task: freeform video description or action JSON",
    )
    parser.add_argument(
        "--question",
        default="Describe the scene and how it changes over time in this frame sequence.",
        help="Question sent with the frame sequence",
    )
    parser.add_argument("--count", type=int, default=20, help="Number of requests")
    parser.add_argument("--hz", type=float, default=0.5, help="Request cadence (default 0.5 Hz)")
    parser.add_argument(
        "--capture-hz",
        type=float,
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Rolling frame window in seconds used for each request",
    )
    parser.add_argument(
        "--video-max-frames",
        type=int,
        default=8,
        help="Cap frames sent per request",
    )
    parser.add_argument(
        "--min-frames",
        type=int,
        default=2,
        help="Minimum buffered frames before sending a request",
    )
    parser.add_argument("--unthreaded", action="store_true", help="Disable threaded latest-frame capture")
    parser.add_argument("--out", default="logs/cosmos_video_probe.jsonl", help="JSONL output path (empty disables)")
    ...
    if not base_url:
        print("cosmos_video_probe: missing base URL (use --base-url or PALA_COSMOS_BASE_URL).")
        return 2
    ...
    video_window_s = max(0.1, float(args.video_window_s))
    min_frames = max(1, int(args.min_frames))
    max_frames = max(1, int(args.video_max_frames))
    video_history: deque[tuple[int, np.ndarray]] = deque()
    print(
        f"cosmos_video_probe: mode={mode} url={chat_url} model={model} hz={args.hz:.2f} "
        f"count={args.count} threaded={is_threaded} task={args.task} "
        f"video_window_s={video_window_s:.1f} video_max_frames={max_frames}"
    )
    ...
    try:
        for idx in range(1, max(1, args.count) + 1):
            ...
            video_history.append((packet.mono_ns, frame))
            cutoff_ns = now_mono_ns - int(video_window_s * 1_000_000_000)
            while video_history and video_history[0][0] < cutoff_ns:
                video_history.popleft()

            frames_for_request, frame_offsets_ms, window_span_ms, frame_shapes = _sample_frames(
                video_history,
                now_mono_ns=now_mono_ns,
                max_frames=max_frames,
            )
            if len(frames_for_request) < min_frames:
                print(f"[{idx:03d}] warmup buffered_frames={len(frames_for_request)} min_frames={min_frames}")
                continue
            ...
            result = ProbeResult(
                ...
                frames_sent=len(image_data_urls),
                frame_offsets_ms=[round(v, 3) for v in frame_offsets_ms],
                window_span_ms=round(window_span_ms, 3),
                error=error,
            )
            ...
            print(
                ...
                f"frames={len(image_data_urls):2d} span={window_span_ms:6.1f}ms jpeg={total_jpeg_bytes:6d}B"
            )
            if (args.print_response or args.task == "describe") and response_preview:
                print(f"      response={response_preview[:240]}")

    except KeyboardInterrupt:
        print("cosmos_video_probe: interrupted")
    finally:
        if out_fh is not None:
            out_fh.close()
        if is_threaded:
    ...
    n = len(total_ms_vals)
    if n == 0:
        print("cosmos_video_probe: no requests completed")
        return 1
