...
def test_behavior_zone_dwell():
    fake_time = _Time([0.0, 0.1, 0.2, 0.3, 1.3, 1.4, 1.5])
    planner = _Planner()
    policy = BehaviorPolicy(planner=planner, dwell_s=1.0, cooldown_s=0.5, clock=fake_time)
...
def test_behavior_forces_interrupt_when_switching_off_persistent_action():
    ...
    policy = BehaviorPolicy(planner=planner, dwell_s=10.0, cooldown_s=10.0, clock=fake_time)
...
def test_behavior_refreshes_after_hold_collapse():
    fake_time = _Time([0.0, 0.1, 0.2, 0.3, 3.2, 3.3, 3.4])
