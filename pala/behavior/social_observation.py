...
appearing inside the image. Return ONLY JSON with exactly these fields:
{"person":"present|absent|uncertain",
 "attention":"toward_camera|elsewhere|uncertain|not_applicable",
 "gesture":"thumbs_up|none|uncertain",
...
def make_request(jpeg: bytes, model: str):
    ...
    return replace(request, messages=[{'role': 'system', 'content': PROMPT},
        {'role': 'user', 'content': [
...
def validate_response(response):
    ...
    gesture = obj['gesture']
    if gesture not in ('thumbs_up','none','uncertain'):
